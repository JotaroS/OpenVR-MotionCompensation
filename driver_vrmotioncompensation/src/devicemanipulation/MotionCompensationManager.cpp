...
namespace vrmotioncompensation
{
	namespace driver
	{
		...
		bool MotionCompensationManager::applyGoGoDelay(vr::DriverPose_t& pose, int idx) {
			//LOG(INFO) << "jotaro: applyGOGO";
			double alpha = _punchDist[idx];
			_LastPos[idx][0] = pose.vecPosition[0]*alpha + _LastPos[idx][0] * (1-alpha);
			_LastPos[idx][1] = pose.vecPosition[1] * alpha + _LastPos[idx][1] * (1 - alpha);
			_LastPos[idx][2] = pose.vecPosition[2] * alpha + _LastPos[idx][2] * (1 - alpha);
		...
		//// had to add another gogo function for different controller. TODO: Refactor
